...
export default function AudioPage() {
  ...
  const {
    ...
    queryKey: ["speakers", "sermon"],
    queryFn: () => getSpeakers("sermon"),
    retry: 3,
    retryDelay: (attemptIndex) => Math.min(1000 * 2 ** attemptIndex, 30000)
  });

  const { data: content, isLoading: contentLoading } = useQuery({
    queryKey: ["content", "sermon", selectedSpeaker?.name],
    queryFn: () => getContentBySpeaker(selectedSpeaker?.name || "", "sermon"),
    ...
    return (
      <div className="max-w-7xl mx-auto px-4 py-6">
        ...
        <h1 className="text-3xl font-bold text-foreground mb-4">
          Sermons by {selectedSpeaker.name}
        </h1>
        {contentLoading ? (
          <Loading size="lg" message="Loading sermons..." />
        ) : (
            ...
            {content && content.length > 0 ? (
              ...
              <div className="col-span-full text-center py-8">
                <p className="text-muted-foreground">
                  No sermons found for {selectedSpeaker.name}.
                </p>
              </div>
            )}
          </div>
